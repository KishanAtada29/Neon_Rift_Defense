...
class AsteroidFleet:
    """Manage the full group of asteroids in the game."""
    
    def __init__(self, game: 'AsteroidRiftDefence'):
        """Initialize the asteroid fleet and fleet movement settings."""
        self.game = game
    ...
    def create_fleet(self):
        ...
        fleet_w, fleet_h = self.calculate_fleet_size(asteroid_w, screen_w, asteroid_h, screen_h)
        x_offset, y_offset = self.calculate_offset(asteroid_w, asteroid_h, screen_w, fleet_w, fleet_h)
    ...
    def _create_rectangle_fleet(self, asteroid_w, asteroid_h, fleet_w, fleet_h, x_offset, y_offset):
        ...
        for row in range(fleet_h):
            for col in range(fleet_w):
                ...
                current_y = asteroid_h * row + y_offset
                if col % 2 == 0 or row % 2 ==0:
                    continue
                self._create_asteroid( current_x,current_y)

    def calculate_offset(self, asteroid_w, asteroid_h, screen_w, fleet_w, fleet_h):
        """Calculate the starting x and y position for the fleet."""
        half_screen = self.settings.screen_h//2
        fleet_horizontal_space = fleet_w * asteroid_w
        fleet_vertical_space = fleet_h * asteroid_h
